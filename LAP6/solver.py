...
class Solver():
    ...
    def __init__(self, mesh: Mesh, elements: list[Element],dimension: int,CL: dict,rho:float, Gamma: float,scheme: str = 'upwind',verbose: bool = False):
        ...
        #Velocity field gradient selection
        self._dimension = dimension #0(x) or 1(y)

        if dimension == 0: # x direction

            #Gradient de la vitesse dans la direction x
            self._velGrad = np.array([self._elements[i]._gradU for i in range(len(self._elements))])

            #terme source 
            self._S = np.array([-self._elements[i]._gradP[0] for i in range(len(self._elements))])

        elif dimension == 1: # y direction

            #Gradient de la vitesse dans la direction y
            self._velGrad = np.array([self._elements[i]._gradV for i in range(len(self._elements))])

            #terme source
            self._S = np.array([-self._elements[i]._gradP[1] for i in range(len(self._elements))])
        else:
            raise Exception("Dimension not supported (0(x) or 1(y))")
        ...
        for i in range(1,iterations+1):
            ...
            for face_i in range(self._numberOfBoundaryFaces):
                ...
                if FrontierType == 'D':
                    ...
                    Sdi = Solver.Sdcross(Pnksi,Pksieta,self._Gamma,dAi,self._velGrad[Eg._index],self._velGrad[Eg._index],e_eta)
            ...
            for face_i in range(self._numberOfBoundaryFaces, self._numberOfFaces):
                ...
                #Terme de diffusion
                Di = Solver.Di(Pnksi,self._Gamma,dAi,dKsi)
                Sdi = Solver.Sdcross(Pnksi,Pksieta,self._Gamma,dAi,self._velGrad[Eg._index],self._velGrad[Ed._index],e_eta)
            ...
            for elem in self._elements:
                elem._V[self._dimension] = self._solution[elem._index]


            if self._dimension == 0: #If the velocity field is in the x direction
                lss = LeastSquareSolver(self._mesh,self._elements,self._CL,field="U")
                lss.solve()
            elif self._dimension == 1: #If the velocity field is in the y direction
                lss = LeastSquareSolver(self._mesh,self._elements,self._CL,field="V")
                lss.solve()
