...
class Solver():
    ...
    def __init__(self, mesh: Mesh, elements: list[Element],velocityField, CL: dict,rho:float, Gamma: float,S,scheme: str = 'upwind',verbose: bool = False):
    ...
    def solve(self,iterations: int = 5):
        ...
        for i in range(1,iterations+1):
            ...
            for face_i in range(self._numberOfBoundaryFaces):
                ...
                #Flux convectif
                if callable(self._velocityField):
                    V = self._velocityField(FaceCenter[0], FaceCenter[1])
                else:
                    V = self._velocityField #Si champs constant
